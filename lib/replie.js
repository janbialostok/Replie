...
var installModule = function (mod, cb) {
	try {
		console.log('calling installModule', mod);
...
var configureConnection = function (socket) {
	...
	map(this.modules, 5, installModule.bind(this))
		.then(results => {
...
var REPLIE = class REPLIE {
	constructor (options) {
		...
		this.replConnectionCallback = (socket) => {
			if (!this.room) {
				if (this.connectionMessage) socket.emit('stdout', this.connectionMessage);
				else socket.emit('stdout', 'Hello World');
			}
			else {
				if (this.connectionMessage) this.io.to(this.room).emit('stdout', this.connectionMessage);
				else this.io.to(this.room).emit('stdout', 'Hello World');
			}	
		};
		...
		this._setStream = function (socket) {
			...
			stream.write = (data) => {
				if (this.room) this.io.to(this.room).emit('stdout', data);
				else socket.emit('stdout', data);
			};
		...
		if (typeof this.namespace === 'string') {
			...
			this.namespaces[this.namespace].on('connection', socket => {
				...
				configureConnection.call(this, socket);
			});
